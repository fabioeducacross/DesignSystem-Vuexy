...
const getAlternatives = item => {
  if (item && Array.isArray(item.questionContent)) {
    const value = item.questionContent
      .filter(question =>
...
<template>
  <div>
      ...
      <b-media class="d-flex align-items-center">
        ...
        <div class="d-flex justify-content-between align-items-center gap-1">
          <div class="flex-1">
            ...
            >
            <br />
            <span class="font-14 text-primary font-medium">{{ quizes.length }} tentativas</span>
          </div>
    ...
    <b-table
      :fields="[
        { key: 'date', label: 'data do acesso', sortable: true },
      ...
      <template v-slot:row-details="row">
        <div>
          <h6 class="text-primary font-semibold font-12 text-center">
            GABARITO DO ACESSO {{ row.item.try }}
          </h6>
