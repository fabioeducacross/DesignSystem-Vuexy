<script setup>
import ImportStudentsEdit from '@/views/pages/records/students/import/edit/Index.vue'
import ImportStudentsHistory from '@/views/pages/records/students/import/history/Index.vue'
import ImportStudentsWizard from '@/views/pages/records/students/import/import-wizard/Index.vue'
import { useImportStudents } from '@/views/pages/records/students/import/useImportStudents'
import { getCurrentInstance, watch } from 'vue'

const {
  isVisible,
  view,
  EView,
  sumImportingUsers,
  editingUser,
  hideImport,
  showHistory,
  hasUnsavedChanges,
  cancelImport,
} = useImportStudents()

const { proxy } = getCurrentInstance()

watch(hasUnsavedChanges, (newValue, oldValue) => {
  if (!oldValue && newValue) {
    proxy.$bus.emit('showConfirmExit', () => cancelImport(true))
  }
})
</script>

<template>
  <b-sidebar
    :visible="isVisible"
    width="727px"
    bg-variant="white"
    shadow
    backdrop
    right
    no-close-on-backdrop
    no-close-on-esc
    header-class="px-0 pb-0 pt-1"
    @hidden="cancelImport(true)"
  >
    <template v-slot:header>
      <div class="flex-1">
        <div class="px-1 flex-1 d-flex align-items-center justify-content-between">
          <h4 class="mb-0">Importar Alunos</h4>
          <span
            class="material-symbols-outlined text-body cursor-pointer"
            @click="cancelImport(false)"
            >close</span
          >
        </div>
        <hr class="flex-1" />
      </div>
    </template>
    <div v-if="showHistory">
      <ImportStudentsHistory />
    </div>

    <div v-else>
      <ImportStudentsWizard v-show="editingUser === null" v-if="view === EView.IMPORT_WIZARD" />

      <ImportStudentsEdit v-if="editingUser !== null" />
    </div>

    <template v-slot:footer>
      <PortalTarget name="footer-mass-import" />
    </template>
  </b-sidebar>
</template>