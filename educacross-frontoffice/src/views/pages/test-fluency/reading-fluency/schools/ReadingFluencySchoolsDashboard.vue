      ...
      <b-row v-if="listData.length" class="match-height">
        <b-col v-for="proficiency in listData" :key="proficiency.proficiency" cols="12" md>
          <b-card
            ...
            :class="`${proficiency.isGreater ? 'card-focus shadow' : ''}`"
          ...
          >
            ...
            <div class="d-flex justify-content-center mt-50">
              <span class="">
                ...
                {{ $filters.formatNumber(data.totalStudents) }} alunos</span
              >
            </div>
          </b-card>
        </b-col>
...
const data = computed(() => store.getters['moduleReadingFluencySchools/cards'])
const loading = computed(() => store.getters['moduleReadingFluencySchools/loadingCards'])
