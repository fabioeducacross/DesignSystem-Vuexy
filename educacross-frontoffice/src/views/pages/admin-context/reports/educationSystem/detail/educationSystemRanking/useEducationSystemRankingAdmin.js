...
const moduleName = 'educationSystemRankingAdminModule'
...
export function useEducationSystemRankingAdmin() {
  const { subject, educationSystemId, book, mission, serie } = useFilters()
  ...
  const fetchRanking = async () => {
    ...
    if (response.status === 200) {
      ...
      if (ranking.value.guideName) mission.value.name = ranking.value.guideName
    } else {
