    ...
    <div v-if="importListEmailError" class="list-imports-email-error">
      <div>
        ...
        <div class="import-email-error-identifier">
          Data:
          <span class="import-email-error-identifier-name">
            {{ handleFormatDate(importListEmailError.date) }}
          </span>
        </div>
        <div class="import-email-error-identifier">
          Autor:
...
const importStudentListEmailErrorById = id => {
  busy.value = true
  getImportStudentListEmailErrorById(id).then(response => {
...
const resendEmails = async () => {
  await resendEmailError(importId.value)
    .then(() => {
      // global modal show (Bootstrap-Vue)
      if (typeof window !== 'undefined' && window.$bvModal) {
        window.$bvModal.show('modal-resend-emails')
      } else if (typeof this !== 'undefined' && this.$bvModal) {
        this.$bvModal.show('modal-resend-emails')
      } else if (window && window.app && window.app.$bvModal) {
        window.app.$bvModal.show('modal-resend-emails')
      }
    })
    .catch(() => {
      // global toast
