<template>
  <div class="customCardPage page-break print-component">
    ...
    >
      ...
      <div class="content1">
        <div class="title">
          <span style="color: #ffbe52">1- </span>Baixar o aplicativo
          {{ name }}
        </div>
      ...
      <div class="content3">
        <div class="title text">Ficou com alguma dúvida?</div>
        <div class="text subtitle">
...
import { getStudentAccessLetter } from '@/services/teacher-context/students/Students.service'
...
onMounted(() => {
  ...
  getWhitelabelInfos()
  handleGetInfos()
  integration.value = accessUser.value !== 'Educacross'
})
...
const accessUser = computed(() => store.getters.accessUserData.User)

const handleGetInfos = async () => {
  loading.value = true
  info.value = route.params
  await getStudentAccessLetter(info.value.studentsIds, info.value.batchAction)
    .then(response => {
...
const getWhitelabelInfos = () => {
  ...
  logo.value = getWhitelabelConfig('SecondaryLogo')
}
</script>
