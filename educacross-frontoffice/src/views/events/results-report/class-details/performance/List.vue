<template>
  ...
  >
    ...
    <template #cell(performanceClass)="{ item }">
      <div class="d-flex align-items-center gap-2">
        <ArrowCompare
          :value-initial="item.performanceClass"
          :value-to-compare="item.performanceInstitution"
        />
        <SemaphoreStatus
          :status-enum="getVariantByRule(item.performanceClass || 0, 'eventLearningPerformance')"
        >
          <span class="whitespace-no-wrap">{{ (item.performanceClass || 0) | formatNumber }}%</span>
        </SemaphoreStatus>
      </div>
    </template>
...
const isLoading = computed(() => {
  return store.getters['resultsReportStudentsPerformance/loading']
})
...
const tableColumns = [
  ...
  { key: 'performanceClass', label: 'Rendimento DA TURMA', sortable: true },
  { key: 'performanceInstitution', label: 'Rendimento DA ESCOLA', sortable: true },
]

const dataTable = computed(() => {
  return store.getters['resultsReportStudentsPerformance/data']
})

const totalData = computed(() => {
  return store.getters['resultsReportStudentsPerformance/total']
})

const changeListTable = ({ currentPage, perPage, sortBy, isSortDirDesc, searchQuery }) => {
  store.commit('resultsReportStudentsPerformance/setParams', {
    Search: searchQuery,
    OrderBy: sortBy,
  ...
  store.dispatch('resultsReportStudentsPerformance/fetch')
}
