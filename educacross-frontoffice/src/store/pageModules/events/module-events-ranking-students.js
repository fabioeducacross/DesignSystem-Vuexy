import { getEventRankingStudent } from '@/services/shared/events/Events.Service.js'
...
export default {
  ...
  actions: {
    async fetch({ commit, rootGetters, state }) {
      const serieId = rootGetters['events/serieId']
      if (!serieId) return
      ...
      getEventRankingStudent(eventId, params)
        .then(({ data }) => {
