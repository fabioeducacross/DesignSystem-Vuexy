import { getEducationSystemReportGuide } from '@/services/shared/educationSystem/EducationSystem.Service.js'
...
export default {
  ...
  actions: {
    fetch({ commit, state }) {
      ...
      getEducationSystemReportGuide(educationSystemId.value, {
        ...
        .then(({ data }) => {
          commit('tableStrongWeak', {
            descriptorsStrong: data.descriptorsStrong.map(item => {
              item.descriptor = {
                tag: item.tag,
                educationalOrganizationName: item.educationalOrganizationName,
                knowledgeAxisName: item.knowledgeAxisName,
                name: item.name,
              }
              return item
            }),
            descriptorsToImprove: data.descriptorsToImprove.map(item => {
              item.descriptor = {
                tag: item.tag,
                educationalOrganizationName: item.educationalOrganizationName,
                knowledgeAxisName: item.knowledgeAxisName,
          ...
          commit(
            ...
            data.activities.map(activity => {
              activity.topicName = activity.activityName
              activity.id = activity.activityId.toString()
              return activity
            }),
          )
          commit('cards', {
