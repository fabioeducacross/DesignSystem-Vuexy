<template>
  <div
    v-if="visibleLyrics && visibleLyrics.length > 0"
    ref="container"
    class="lyrics-container"
    :class="{ 'fullscreen-lyrics': isFullscreen }"
  >
    <div class="lyrics-wrapper" :style="{ transform: `translateY(-${scrollOffset}px)` }">
      <div
        v-for="(lyric, index) in visibleLyrics"
        :key="`${index}-${lyric.isActive ? 'active' : 'inactive'}-${lyric.text.substring(0, 10)}`"
        :ref="lyric.isActive ? 'activeLine' : null"
        class="lyric-line"
        :class="{
          active: lyric.isActive,
          upcoming: !lyric.isActive && lyric.start > getCurrentTime(),
          passed: !lyric.isActive && lyric.end < getCurrentTime(),
        }"
        v-html="lyric.text"
      ></div>

      <!-- Extra padding for smooth scrolling -->
      <div class="lyrics-padding" :class="{ 'fullscreen-padding': isFullscreen }"></div>
    </div>

    <!-- Spotify-style controls overlay -->
    <div v-if="!isFullscreen" class="lyrics-controls">
      <b-button
...
const activeLineIndex = computed(() =>
  props.visibleLyrics.findIndex(lyric => lyric.isActive)
)

// Helper methods
const getCurrentTime = () => props.currentTime

const getLineProgress = lyric => {
  if (!lyric || !lyric.isActive) return 0
  const duration = lyric.end - lyric.start
  const elapsed = props.currentTime - lyric.start
  const progress = Math.max(0, Math.min(100, (elapsed / duration) * 100))
  return progress
}

const updateProgress = () => {
  const activeLyric = props.visibleLyrics.find(lyric => lyric.isActive)
  if (activeLyric) {
    currentLineProgress.value = getLineProgress(activeLyric)
  } else {
...
const scrollToActiveLine = () => {
  let activeLineEls = activeLine.value
  if (activeLineEls && activeLineEls[0]) {
    const activeLineEl = activeLineEls[0]
    ...
    if (containerEl) {
      ...
      let optimalOffset =
        activeLineEl.offsetTop - containerHeight / 2 + lineHeight / 2

      if (props.isFullscreen) {
        const paddingTop = containerHeight * 0.4
        optimalOffset = activeLineEl.offsetTop - paddingTop
      }

      scrollOffset.value = Math.max(0, optimalOffset)
    }
  }
}
