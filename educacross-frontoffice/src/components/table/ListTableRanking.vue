...
const { MODULE_NAME, exportExcel } = useMissionPlusRanking()

const listTableRef = ref(null)

const loading = computed(() => store.getters[`${MODULE_NAME}/loading`])
...
const tableColumns = [
  ...
  { key: 'stars', label: 'Estrelas', sortable: true },
  { key: 'pointsXp', label: 'Pontos XP', sortable: true },
...
const showPosition = item => {
  if (!item || !item.classPosition) return '-'
