 ...
 */
export function parseVueComponent(code, filename = 'Component.vue') {
 ...
 */
function extractPropsFromScriptSetup(code) {
    const props = [];
    ...
    if (typedPropsMatch) {
        ...
        const sourceFile = ts.createSourceFile('temp.ts', propsInterface, ts.ScriptTarget.Latest, true);
 ...
 */
function extractEventsFromScriptSetup(code) {
    const events = [];
    // Buscar defineEmits<{ ... }>()
...
//# sourceMappingURL=vue-parser-poc.js.map