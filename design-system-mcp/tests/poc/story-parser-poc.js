 ...
 */
export function parseStoryFile(code) {
            ...
            for (const decl of node.declaration.declarations) {
                if (t.isVariableDeclarator(decl) && t.isIdentifier(decl.id)) {
                    ...
                    if (t.isObjectExpression(decl.init)) {
                        const argsProperty = decl.init.properties.find(p => t.isObjectProperty(p) &&
 ...
 */
export function extractStoryNames(code) {
    const storyNames = [];
...
//# sourceMappingURL=story-parser-poc.js.map